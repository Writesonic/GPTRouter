...
export default async function checkOpenaiHealth(orm: DataSource) {
    ...
    const promises = openaiModels.map(async (model) => {
        ...
        const obj = await orm.getRepository(ModelHealthCheck).findOne({ where: { modelId: model.id, providerId: openaiProvider.id } });

        try {
            const startTime = Date.now();
            const response = await generateResponse({ params, timeout: DEFAULT_TIMEOUT_IN_MS, maxRetries: DEFAULT_MAX_RETRIES });
