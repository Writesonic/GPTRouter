...
import { BasePromptParams, CreatePromptSchema, DeletePromptSchema, GetAllPromptsSchema, GetPromptByIdSchema, OptimizePromptSchema, Roles, UpdatePromptSchema } from "../schema"
...
export const getPromptById = async (
    ...
    try {
        ...
        if (!prompt) {
            ...
            const prompts = await request.server.orm.getRepository(Prompt).find({ where: { parentPromptId: id, isDeleted: false, } })
...
export const optimizePrompt = async (
    ...
    try {
        ...
        const selectedModel = await request.server.orm.getRepository(Model).findOne({ where: { name: "gpt-4-0613" }, relations: ["provider"] })
        let newMessages: any = []
        if (selectedModel) {
