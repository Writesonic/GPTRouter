...
import { GenerateFromImageResponseSchema, GenerateFromPromptResponseSchema, GenerateResponseSchema, Roles, GenerateImageResponseSchema } from "../schema"
...
import { DEFAULT_MAX_RETRIES, DEFAULT_TIMEOUT_IN_MS, ERROR_MESSAGES, ImageChatModels, ImageVendor, MODEL_LATENCY_LIMIT_FOR_GENERATION, Providers, SSE_EVENTS, STABLE_DIFFUSION_API_URL, STABLE_DIFFUSION_IMAGE_GENERATION_PARAMS, STABLE_DIFFUSION_IMAGE_GENERATION_PARAMS_HEIGHT, STABLE_DIFFUSION_IMAGE_GENERATION_PARAMS_WIDTH, STABLE_DIFFUSION_TEXT_TO_IMAGE } from "../constants"
...
export const generateResponse = async (
    ...
    try {
        ...
        for (const params of data) {
            ...
            if (!!!provider) {
                throw new ApiError(400, `Provider with ${params.providerId ? `id ${params.providerId}` : `name ${params.providerName}`} does not exist`);
            }
            ...
            if (!!!model) {
                throw new ApiError(400, `Model with ${params.modelId ? `id ${params.modelId}` : `name ${params.modelName}`} does not exist`);
            }

            const obj = await orm.getRepository(ModelHealthCheck).findOne({ where: { modelId: model.id, providerId: provider.id }, order: { lastChecked: 'DESC' } })
            ...
            if (stream) {
                ...
                reply.sse((async function* source() {
                        ...
                        for await (const message of response) {
                            if (message.event === SSE_EVENTS.META) {
                                try {
                                    ...
                                    await request.server.orm.getRepository(ModelUsage).create({
                                        modelId: model?.id,
                                        providerId: provider?.id,
                                        promptTokens: usage?.prompt_tokens,
                                        completionTokens: usage?.completion_tokens,
                                        totalTokens: usage?.total_tokens,
                                        cost: (model?.cost?.input * (usage?.prompt_tokens ?? 0) + model?.cost?.output * (usage?.completion_tokens ?? 0)) / model?.cost?.factor,
                                        // @ts-ignore
                    ...
                    yield ({
                        data: JSON.stringify({
                            event: SSE_EVENTS.GENERATION_SOURCE, data: {
                                modelId: model.id,
                                providerId: provider.id,
                            }
                        })
...
export const generateResponseFromPrompt = async (
    ...
    try {
        ...
        const prompt = await request.server.orm.getRepository(Prompt).findOne({ where: { id: request?.body?.data?.promptId } });
        ...
        if (inputData) {
            ...
            if (prompt?.promptParams?.prompt) {
                ...
                if (matches) {
                    matches.forEach((match) => {
                        const key = match.replace('[', '').replace(']', '');
                        prompt.promptParams.prompt = prompt.promptParams.prompt.replace(match, inputData[key as keyof typeof inputData]);
                    });
                }
                if (language) {
        ...
        for (const modelId of modelIds) {
            ...
            const obj = await orm.getRepository(ModelHealthCheck).findOne({ where: { modelId: modelId, providerId: model.providerId }, order: { lastChecked: 'DESC' } })
...
const generateImagesWithDALL_E = async (request: FastifyRequestTypebox<typeof GenerateImageResponseSchema>, reply: FastifyReplyTypebox<typeof GenerateImageResponseSchema>) => {
    ...
    try {
        ...
        if (!imageData.length) {
            throw new ApiError(
                406, "Sorry our AI is currently overloaded please try again"
            )
        }
...
const generateImagesWithStableDiffusion = async (request: FastifyRequestTypebox<typeof GenerateImageResponseSchema>, reply: FastifyReplyTypebox<typeof GenerateImageResponseSchema>) => {
    try {
